...
def open_gripper_at_pose(inputs, f_pc_original):
    ...
    f_pc = tfg_transformation.rotation_matrix_3d.rotate(
        f_pc_original, tf_from_euler) + trans

    original_normal = tf.constant([1., 0., 0.], dtype=tf.float32)
    gripper_normal = tfg_transformation.rotation_matrix_3d.rotate(
        original_normal, tf_from_euler)
...
def f_verts_at_pos(inputs, gripper_pos, f_pc_original):
    ...
    f1_verts, f2_verts, gripper_normal = open_gripper_at_pose(
        inputs, f_pc_original)
...
def gripper_world_pos(inputs, f_pc_original):
    ...
    f_verts, gripper_normal = f_verts_at_pos(inputs, inputs['gripper_pos'][0],
                                             f_pc_original)
    f_verts_next, _ = f_verts_at_pos(inputs, inputs['target|gripper_pos'][0],
                                     f_pc_original)

    # Get velocity of each gripper
    num_verts_per_f = f_verts.shape[0] // 2

    f1_force_vecs = tf.tile(-1. * gripper_normal, [num_verts_per_f, 1]) * (
        inputs['target|force'] - inputs['force'])

    f2_force_vecs = tf.tile(gripper_normal, [num_verts_per_f, 1]) * (
        inputs['target|force'] - inputs['force'])
    ...
    unit_f_force_vecs = tf.concat((unit_f1_force_vecs, unit_f2_force_vecs),
                                  axis=0)
...
signature_dict = {
    "name":
    tf.TensorSpec(shape=[traj_len, bs, 1], dtype=tf.string, name="name"),
    "cutoffs":
    tf.TensorSpec(shape=[traj_len, bs, 5], dtype=tf.float32, name="cutoffs"),
    "cells":
    tf.TensorSpec(shape=[traj_len, None, 4], dtype=tf.int32, name="cells"),
    "force":
    tf.TensorSpec(shape=[traj_len, bs, 1], dtype=tf.float32, name="force"),
    "gripper_pos":
    tf.TensorSpec(shape=[traj_len, bs, 2],
                  dtype=tf.float32,
                  name="gripper_pos"),
    "mesh_edges":
    tf.TensorSpec(shape=[traj_len, None, 2], dtype=tf.int32,
                  name="mesh_edges"),
    "mesh_pos":
    tf.TensorSpec(shape=[traj_len, None, 3], dtype=tf.float32,
                  name="mesh_pos"),
    "node_mod":
    tf.TensorSpec(shape=[traj_len, None, 1], dtype=tf.float32,
                  name="node_mod"),
    "node_type":
    tf.TensorSpec(shape=[traj_len, None, 1], dtype=tf.int32, name="node_type"),
    "pd_stress":
    tf.TensorSpec(shape=[traj_len, None, 1],
                  dtype=tf.float32,
                  name="pd_stress"),
    "sim_world_pos":
    tf.TensorSpec(shape=[traj_len, None, 3],
                  dtype=tf.float32,
                  name="sim_world_pos"),
    "stress":
    tf.TensorSpec(shape=[traj_len, None, 1], dtype=tf.float32, name="stress"),
    "target|force":
    tf.TensorSpec(shape=[traj_len, bs, 1],
                  dtype=tf.float32,
                  name="target|force"),
    "target|gripper_pos":
    tf.TensorSpec(shape=[traj_len, bs, 2],
    ...
    tf.TensorSpec(shape=[traj_len, None, 3],
                  dtype=tf.float32,
                  name="target|world_pos"),
    "tfn":
    tf.TensorSpec(shape=[traj_len, bs, 6], dtype=tf.float32,
                  name="tfn"),  # (1, 6,1)
    "world_edges":
    tf.TensorSpec(shape=[traj_len, None, 2],
                  dtype=tf.int32,
                  name="world_edges"),
    "world_pos":
    tf.TensorSpec(shape=[traj_len, None, 3],
                  dtype=tf.float32,
                  name="world_pos"),
    "world_edges_over_force":
    tf.TensorSpec(shape=[traj_len, None, 1],
                  dtype=tf.float32,
                  name="world_edges_over_force"),
    "velocity":
    tf.TensorSpec(shape=[traj_len, None, 3], dtype=tf.float32,
                  name="velocity"),
}
...
class Model(snt.Module):
    ...
    def __init__(self, learned_model, FLAGS, name='Model'):
        ...
        self._edge_normalizer = normalization.Normalizer(
            size=edge_feature_size,
            name='edge_normalizer')  # 2*(3D coord  + length) = 8

        ######### World edge #########
        world_edge_feature_size = 5  # 3D coord  + length + curr_force = 5
    ...
    def accumulate_stats(self, inputs):
        ...
        self._build_graph(
            ...
            is_training=True,
        )  # is_training here is = accumulate
    ...
    def _build_graph(self, inputs, use_precomputed, is_training):
        ...
        actuator_mask = tf.expand_dims(
            tf.equal(inputs['node_type'][:, 0], common.NodeType.OBSTACLE), -1)

        #######################################################################################
        # Calculate world pos partially from gripper_pos
        if not utils.using_dm_dataset(self.FLAGS) and not use_precomputed:
            f_verts, f_verts_next, f_force_vecs, unit_f_force_vecs = gripper_world_pos(
                inputs, self.f_pc_original)
            ...
            world_pos = tf.concat(
                [f_verts, inputs['world_pos'][1180:, :]],
                axis=0)  # This should be the same as inputs['world_pos']
        ...
        node_mod_features = tf.where(actuator_mask, node_mod_exp * 0,
                                     node_mod_exp)
        ...
        if self.FLAGS.mod == "edge":
            ...
            mesh_edge_features = tf.concat(
                [mesh_edge_features, mesh_edge_mod_features], axis=-1)

        mesh_edges = core_model.EdgeSet(name='mesh_edges',
                                        features=self._edge_normalizer(
                                            mesh_edge_features, is_training),
                                        receivers=receivers,
                                        senders=senders)

        #################### World edge features #########
        if not use_precomputed and self.FLAGS.compute_world_edges:
            world_senders, world_receivers = common.construct_world_edges(
                world_pos, inputs['node_type'], self.FLAGS)

        else:
            world_senders, world_receivers = common.sr_world_edges(
                inputs['world_edges'])

        # Force at nodes or edges
        if self.FLAGS.use_total_force_as_feature:
            force_label = inputs['force'][
                0, 0]  # Label every node/edge with total gripper force
            force_change_label = inputs['target|force'][0,
                                                        0] - inputs['force'][0,
                                                                             0]

        else:
            force_label = inputs['force'][0, 0] / (
                0.5 * tf.cast(tf.shape(world_senders)[0], tf.float32)
            )  # Label every node/world edge with total gripper force / num contacts
            force_change_label = (inputs['target|force'][0, 0] -
                                  inputs['force'][0, 0]) / (0.5 * tf.cast(
                                      tf.shape(world_senders)[0], tf.float32))

        relative_world_pos = (tf.gather(world_pos, world_senders) -
                              tf.gather(world_pos, world_receivers))

        relative_world_norm = tf.norm(relative_world_pos,
                                      axis=-1,
                                      keepdims=True)

        if not use_precomputed:
            world_edge_force_feature = tf.fill(tf.shape(relative_world_norm),
                                               force_label)
            world_edge_force_change_feature = tf.fill(
                tf.shape(relative_world_norm), force_change_label)

        else:
            world_edge_force_feature = inputs['world_edges_over_force']

        world_edge_features = tf.concat([
            relative_world_pos, relative_world_norm, world_edge_force_feature
        ],
                                        axis=-1)

        world_edges = core_model.EdgeSet(name='world_edges',
                                         features=self._world_edge_normalizer(
                                             world_edge_features, is_training),
        ...
        if not use_precomputed:
            ...
            num_verts_total = tf.shape(
                inputs['mesh_pos'])[0]  #.get_shape().as_list()[0]
            pad_diff = num_verts_total - num_f_verts_total
            paddings = [[0, pad_diff], [0, 0]]
        ...
        if self.FLAGS.mod == "node":
            node_features = tf.concat([node_features, node_mod_features],
                                      axis=-1)

        return core_model.MultiGraph(node_features=self._node_normalizer(
    ...
    def __call__(self,
        ...
        return self._update(inputs, use_precomputed, per_node_network_output,
    ...
    def get_output_targets(self, inputs):
        ...
        target_position_change = inputs['sim_world_pos'] - inputs[
            'world_pos']  # where inputs['world_pos'] is the initial world pos at the start of trajectory
        zero_position_change = inputs['world_pos'] - inputs['world_pos']
        object_mask = tf.expand_dims(
            tf.not_equal(inputs['node_type'][:, 0], common.NodeType.OBSTACLE),
            -1)
        target_position_change_from_initial = tf.where(object_mask,
                                                       target_position_change,
                                                       zero_position_change)

        combined_target = tf.concat(
            [target_position_change_from_initial, target_stress_change], 1)
    ...
    def loss(self, inputs, use_precomputed, normalize=True, accumulate=False):
        ...
        graph = self._build_graph(
            inputs, use_precomputed, is_training=accumulate
        )  # is_training used to always be True -> accumulate always True
        ...
        if normalize:
            ...
            object_output, stress_output = tf.split(
                self._output_normalizer.inverse(network_output), [3, 1], 1)

        # build loss
        if utils.using_dm_dataset(
                self.FLAGS
        ):  # If using DM dataset, we care about positions only for soft nodes
            object_mask = tf.equal(inputs['node_type'][:, 0],
                                   common.NodeType.NORMAL)

        else:  # Else if ABC, we want to care about errors for soft nodes and surface nodes (0 and 2)
            object_mask = tf.not_equal(inputs['node_type'][:, 0],
                                       common.NodeType.OBSTACLE)

        # Normalize target position change and stress first
        if normalize:
            target_normalized = self._output_normalizer(
                combined_target, accumulate)
        else:
            target_normalized = combined_target

        # Combined position and stress losses
        object_target_normalized, stress_target_normalized = tf.split(
            target_normalized, [3, 1], 1)
        if self.FLAGS.loss_function.lower() == "mse":
            stress_error = tf.reduce_sum(
                (stress_target_normalized - stress_output)**2, axis=1)
        elif self.FLAGS.loss_function.lower() == "mae":
            stress_error = tf.reduce_sum(tf.math.abs(stress_target_normalized -
                                                     stress_output),
                                         axis=1)
        else:
            assert (False)
        ...
        object_error = tf.reduce_sum(
            (object_target_normalized - object_output)**2, axis=1)
    ...
    def _update(self,
        ...
        actuator_mask = tf.expand_dims(
            tf.not_equal(inputs['node_type'][:, 0], common.NodeType.OBSTACLE),
            -1)  # Where nodes are NOT actuators
        fixed_points = tf.expand_dims(
            tf.equal(inputs['node_type'][:, 0], common.NodeType.HANDLE),
            -1)  # Where nodes ARE fixed

        if not utils.using_dm_dataset(self.FLAGS) and not use_precomputed:

            f_verts, f_verts_next, _, _ = gripper_world_pos(
                inputs, self.f_pc_original)
            num_f_verts_total = 1180  # tf.shape(f_verts)[0]
            num_verts_total = tf.shape(inputs['mesh_pos'])[0]
            pad_diff = num_verts_total - num_f_verts_total
            ...
            curr_pos_gt = tf.where(actuator_mask, inputs['world_pos'],
                                   finger_curr_pos_gt)
            next_pos_gt = tf.where(actuator_mask, inputs['target|world_pos'],
                                   finger_next_pos_gt)
            position_change_gt = next_pos_gt - curr_pos_gt

        else:
            position_change_gt = inputs['target|world_pos'] - inputs[
                'world_pos']  # If not calculating from gripper pos

        # Get predictions
        position_change, stress_change = tf.split(
            self._output_normalizer.inverse(per_node_network_output), [3, 1],
            1)
        ...
        position_change = tf.where(fixed_points, zero_position_change,
                                   position_change)

        position_change = tf.where(actuator_mask, position_change,
                                   zero_position_change)
        ...
        stress_change = tf.where(
            actuator_mask, stress_change,
            zero_stress)  #Got rid of this for fake batching
